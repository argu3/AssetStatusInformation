...
namespace AssetStatusInfo
{
    ...
    internal class SearchGridDecorator
    {
        ...
        public Action actionsOnSearchClear;
        ...
        public SearchGridDecorator(DataGridView SearchableGrid, DataGridView SearchGrid, Func<DataTable> ResetSearchableGridData)
        {
            ...
            actionsOnSearchClear += MatchColumnWidth;
        }
        ...
        private void SearchGrid_CellClick(object? sender, DataGridViewCellEventArgs e)
        {
            if (sender != null)
            {
                if (sender.GetType() == typeof(DataGridView) && e.RowIndex != -1 && e.ColumnIndex != -1)
                {
        ...
        public void AddSearchClearAction(Action SearchAction)
        {
            actionsOnSearchClear += SearchAction;
        }

        public void RemoveSearchClearAction(Action SearchAction)
        {
            //original
            if (actionsOnSearchClear == null ? false : actionsOnSearchClear.GetInvocationList().Contains(SearchAction))
            {
                if (SearchAction != null)
                {
                    actionsOnSearchClear -= SearchAction;
